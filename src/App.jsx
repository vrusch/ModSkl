        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700 mb-6">
          ...
          {user && !user.isAnonymous ? (
            <div className="space-y-3">
              ...
              <button
                onClick={onLogout}
                className="w-full bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200 border border-slate-700 hover:border-slate-600 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
              >
            ...
            <div>
              <button
                onClick={onLogin}
                className="w-full bg-white hover:bg-slate-200 text-slate-900 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
              >
...
export default function App() {
  ...
  const handleLogout = async () => {
    try {
      await signOut(auth);
      window.location.reload(); // Reload to re-trigger anonymous auth or clean state
    } catch (error) {
